...
scplot(exampleA1B1A2B2) %>% 
  ...
  set_yaxis(color = "sienna3", size = 3) %>%
  ...
  add_marks(case = "all", positions = 'points < quantile(points, 0.1)', 
            color = "blue3", size = 1.5) %>%
  add_marks(positions = outlier(exampleABAB), color = "brown", size = 2) %>%
  add_text(case = 1, x = 5, y = 35, "Interesting", 
           color = "darkgreen", angle = 20, size = 0.5) %>%
