...
scplot(exampleABC) %>% 
  ...
  add_labels(text = list(color = "black", size = 1), background = list(fill = "grey98"), nudge_y = 15) %>%
...
scplot(example_A24) %>% 
  ...
  set_theme_element(legend.background = element_rect(color = "grey50"))
...
scplot(exampleAB_add) %>%
  ...
  set_theme_element(
   ...
   add_labels(variable = "cigarrets", nudge_y = 3, 
              text = list(color = "blue", size = 0.5)) %>%
    add_labels(nudge_y = 3, text = list(color = "black", size = 0.5),
...
scplot(exampleA1B1A2B2) %>% 
  ...
  set_casenames(sample_names(3), text = list(color = "steelblue4", size = 0.7)) %>%
