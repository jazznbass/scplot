...
```{r}
scplot(exampleABC) %>%
  add_arrow(case = 1, 6, 90, 3, 63) %>%
  add_text("Problem", case = 1, x = 6, y = 94, color = "red", size = 1 ) 
```
...
```{r}
scplot(exampleABC) %>%
  set_panel(fill = "grey90", color = "black", size = 2)
```

### Different panel color for each phase
